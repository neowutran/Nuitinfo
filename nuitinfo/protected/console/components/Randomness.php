...
class Randomness
{
    ...
    public static function substr($string, $start = 0, $length = null)
    {
        ...
        return function_exists('mb_substr') ? mb_substr($string, $start, $length, 'ISO-8859-1') : substr(
            $string,
            $start,
            $length
        );
    ...
    public static function warn($msg)
    {
        ...
        if (class_exists('Yii')) /** @noinspection PhpUndefinedClassInspection */ {
    ...
    public static function randomBytes($length = 8, $cryptoStrong = true, $http = false)
    {
        ...
        if ($cryptoStrong) {
            ...
            if (function_exists('openssl_random_pseudo_bytes') && false !== ($s = openssl_random_pseudo_bytes(
                    $length,
                    $safe
                )) && $safe && self::strlen($s) >= $length
            ) {
            ...
            if (function_exists('mcrypt_create_iv') && false !== ($s = mcrypt_create_iv(
                    $length,
                    MCRYPT_DEV_RANDOM
            ...
            if (false !== ($f = @fopen('/dev/random', 'r')) && stream_set_blocking(
                    $f,
                    0
                ) && false !== ($s = @fread($f, $length)) && (fclose($f) || true) && self::strlen(
                    $s
                ) >= $length
            ) {
                return self::substr($s, 0, $length);
            ...
            $i = 0;
            while (self::strlen($s) < $length && false !== ($r = self::sessionBlock()) && ++$i < 3) {
                $s .= $r;
            }
            ...
            // Try http://random.org
            if (self::strlen($s) < $length && $http && false !== ($r = @file_get_contents(
    ...
    public static function blowfishSalt($cost = 10, $cryptoStrong = false)
    {
        ...
        return '$2a$' . str_pad($cost, 2, '0', STR_PAD_RIGHT) . '$' . strtr(
