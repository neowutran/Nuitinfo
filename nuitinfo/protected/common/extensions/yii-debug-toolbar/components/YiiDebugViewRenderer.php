...
class YiiDebugViewRenderer extends ProxyComponent
{
    ...
    protected function collectDebugInfo($context, $sourceFile, $data)
    {
        ...
        while ($backTraceItem = array_shift($backTrace)) {
            if (isset($backTraceItem['object']) && $backTraceItem['object'] && is_a(
                    $backTraceItem['object'],
                    get_class($context)
                ) && in_array(
        ...
        array_push(
            $this->_debugStackTrace,
            array(
                'context'           => $context,
                'contextProperties' => get_object_vars($context),
                'action'            => $context instanceof CController ? $context->action : null,
                'actionParams'      => ($context instanceof CController && method_exists(
                            $context,
                            'getActionParams'
                        )) ? $context->actionParams : null,
