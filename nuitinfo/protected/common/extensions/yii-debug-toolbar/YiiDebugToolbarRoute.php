...
class YiiDebugToolbarRoute extends CLogRoute
{
    ...
    private $_toolbarWidget, $_startTime, $_endTime;
    ...
    public function init()
    {
        ...
        $this->enabled && $this->enabled = ($this->allowIp(Yii::app()->request->userHostAddress) && !Yii::app(
            )->getRequest()->getIsAjaxRequest());
    ...
    protected function allowIp($ip)
    {
        ...
        foreach ($this->ipFilters as $filter) {
            ...
            if (preg_match('/^[\d\.]*\*?$/', $filter)) {
                ...
                if (preg_match('/^([\d\.]+)\/(\d+)$/', $filter, $match)) {
