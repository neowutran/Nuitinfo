...
class YiiDebugToolbarResourceUsage extends CWidget
{
    ...
    public function run()
    {
        ...
        $resources = array(
            ...
            YiiDebug::t('Memory Peak Usage') => function_exists('memory_get_peak_usage') ? number_format(
                        memory_get_peak_usage() / 1024
                    ) . ' KB' : 'N/A',
        );

        if (function_exists('mb_strlen') && isset($_SESSION)) {
            $resources[YiiDebug::t('Session Size')] = sprintf(
                '%0.3F KB',
                mb_strlen(serialize($_SESSION)) / 1024
            );
        }
