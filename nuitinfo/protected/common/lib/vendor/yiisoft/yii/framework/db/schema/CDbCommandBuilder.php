...
class CDbCommandBuilder extends CComponent
{
    ...
    public function createInsertCommand($table, $data)
    {
        ...
        $sql     = "INSERT INTO {$table->rawName} (" . implode(', ', $fields) . ') VALUES (' . implode(
                ', ',
                $placeholders
            ) . ')';
        $command = $this->_connection->createCommand($sql);
    ...
    protected function composeMultipleInsertCommand($table, array $data, array $templates = array())
    {
        ...
        foreach ($data as $rowKey => $rowData) {
            ...
            foreach ($columns as $columnName) {
                ...
                $columnValue = array_key_exists(
                    $columnName,
                    $rowData
                ) ? $rowData[$columnName] : new CDbExpression('NULL');
    ...
    public function createSearchCondition($table, $columns, $keywords, $prefix = null, $caseSensitive = true)
    {
        ...
        foreach ($columns as $name) {
            ...
            foreach ($keywords as $keyword) {
                ...
                if ($caseSensitive) {
                    $condition[] = $prefix . $column->rawName . ' LIKE ' . $this->_connection->quoteValue(
                            '%' . $keyword . '%'
                        );
                } else {
                    $condition[] = 'LOWER(' . $prefix . $column->rawName . ') LIKE LOWER(' . $this->_connection->quoteValue(
                            '%' . $keyword . '%'
                        ) . ')';
                }
            }
