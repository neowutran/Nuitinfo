...
class CDateTimeParser
{
        ...
        foreach ($tokens as $token) {
            switch ($token) {
                ...
                default:
                    ...
                    if ($i >= $n || ($token{0} != '?' && (self::$_mbstringAvailable ? mb_substr(
                                $value,
                                $i,
                                $tn,
                                Yii::app()->charset
                            ) : substr($value, $i, $tn)) !== $token)
                    ) {
                        return false;
                    }
                    $i += $tn;
    ...
    private static function tokenize($pattern)
    {
        ...
        for ($start = 0, $i = 1; $i < $n; ++$i) {
            ...
            if ($c !== $c0) {
                $tokens[] = self::$_mbstringAvailable ? mb_substr(
                    $pattern,
                    $start,
                    $i - $start,
                    Yii::app()->charset
                ) : substr($pattern, $start, $i - $start);
                $c0       = $c;
                $start    = $i;
            }
        }
        $tokens[] = self::$_mbstringAvailable ? mb_substr($pattern, $start, $n - $start, Yii::app()->charset) : substr(
            $pattern,
            $start,
            $n - $start
        );
    ...
    protected static function parseInteger($value, $offset, $minLength, $maxLength)
    {
        ...
        for ($len = $maxLength; $len >= $minLength; --$len) {
            $v = self::$_mbstringAvailable ? mb_substr($value, $offset, $len, Yii::app()->charset) : substr(
                $value,
                $offset,
                $len
            );
            if (ctype_digit($v) && (self::$_mbstringAvailable ? mb_strlen($v, Yii::app()->charset) : strlen(
                    $v
                )) >= $minLength
            ) {
                return $v;
    ...
    protected static function parseAmPm($value, $offset)
    {
        ...
        $v = strtolower(
            self::$_mbstringAvailable ? mb_substr($value, $offset, 2, Yii::app()->charset) : substr($value, $offset, 2)
        );
    ...
    protected static function parseMonth($value, $offset, $width, &$monthName)
    {
        ...
        for ($len = 1; $offset + $len <= $valueLength; $len++) {
            $monthName = self::$_mbstringAvailable ? mb_substr($value, $offset, $len, Yii::app()->charset) : substr(
                $value,
                $offset,
                $len
            );
            ...
            {
                $monthName = self::$_mbstringAvailable ? mb_substr($monthName, 0, -1, Yii::app()->charset) : substr(
                    $monthName,
                    0,
                    -1
                );
        ...
        $monthName = self::$_mbstringAvailable ? mb_strtolower($monthName, Yii::app()->charset) : strtolower(
            $monthName
        );
        ...
        foreach ($monthNames as $k => $v) {
            $monthNames[$k] = rtrim(
                self::$_mbstringAvailable ? mb_strtolower($v, Yii::app()->charset) : strtolower($v),
                '.'
            );
        }
        ...
        foreach ($monthNamesStandAlone as $k => $v) {
            $monthNamesStandAlone[$k] = rtrim(
                self::$_mbstringAvailable ? mb_strtolower($v, Yii::app()->charset) : strtolower($v),
                '.'
            );
        }

        if (($v = array_search($monthName, $monthNames)) === false && ($v = array_search(
                $monthName,
