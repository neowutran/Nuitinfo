...
class CFileValidator extends CValidator
{
    ...
    protected function validateAttribute($object, $attribute)
    {
        ...
        if ($this->maxFiles > 1) {
            ...
            if (count($files) > $this->maxFiles) {
                $message = $this->tooMany !== null ? $this->tooMany : Yii::t(
                    'yii',
    ...
    protected function validateFile($object, $attribute, $file)
    {
        ...
        if ($error == UPLOAD_ERR_INI_SIZE || $error == UPLOAD_ERR_FORM_SIZE || $this->maxSize !== null && $file->getSize(
            ) > $this->maxSize
        ) {
            $message = $this->tooLarge !== null ? $this->tooLarge : Yii::t(
                'yii',
        ...
        if ($this->types !== null) {
            ...
            if (!in_array(strtolower($file->getExtensionName()), $types)) {
                $message = $this->wrongType !== null ? $this->wrongType : Yii::t(
                    'yii',
        ...
        if ($this->mimeTypes !== null && !empty($file->tempName)) {
            ...
            if ($mimeType === false || !in_array(strtolower($mimeType), $mimeTypes)) {
                $message = $this->wrongMimeType !== null ? $this->wrongMimeType : Yii::t(
                    'yii',
                ...
                $this->addError(
                    ...
                    $attribute,
                    $message,
                    array('{file}' => $file->getName(), '{mimeTypes}' => implode(', ', $mimeTypes))
                );
            }
        }
    }
