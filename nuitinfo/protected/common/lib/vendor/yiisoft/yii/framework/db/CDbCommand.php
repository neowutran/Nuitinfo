...
class CDbCommand extends CComponent
{
    ...
    public function execute($params = array())
    {
        ...
        try {
            if ($this->_connection->enableProfiling) {
                Yii::beginProfile(
                    'system.db.CDbCommand.execute(' . $this->getText() . $par . ')',
    ...
    private function queryInternal($method, $mode, $params = array())
    {
        ...
        try {
            if ($this->_connection->enableProfiling) {
                Yii::beginProfile(
                    'system.db.CDbCommand.query(' . $this->getText() . $par . ')',
            ...
            foreach ($columns as $i => $column) {
                if (is_object($column)) {
                    ...
                    if (preg_match('/^(.*?)(?i:\s+as\s+|\s+)(.*)$/', $column, $matches)) {
                        $columns[$i] = $this->_connection->quoteColumnName(
                                $matches[1]
        ...
        if (is_string($tables) && strpos($tables, '(') !== false) {
            ...
            foreach ($tables as $i => $table) {
                if (strpos($table, '(') === false) {
                    ...
                    {
                        $tables[$i] = $this->_connection->quoteTableName(
                                $matches[1]
    ...
    public function addForeignKey($name, $table, $columns, $refTable, $refColumns, $delete = null, $update = null)
    {
        ...
        return $this->setText(
            $this->getConnection()->getSchema()->addForeignKey(
                $name,
                $table,
                $columns,
                $refTable,
    ...
    public function createIndex($name, $table, $column, $unique = false)
    {
        ...
        return $this->setText(
            $this->getConnection()->getSchema()->createIndex($name, $table, $column, $unique)
        )->execute();
    }
