...
EOD;
    ...
    public function run($args)
    {
        ...
        if (isset($args[1])) {
            ...
            if (($pos = strrpos($controllerID, '/')) === false) {
                ...
                $controllerFile = $module->controllerPath . DIRECTORY_SEPARATOR . str_replace(
                        '/',
                        DIRECTORY_SEPARATOR,
                        $controllerFile
                    );
        ...
        $functionalTestPath = $this->functionalTestPath === null ? Yii::getPathOfAlias(
            'application.tests.functional'
        ) : $this->functionalTestPath;

        $viewPath       = $module->viewPath . DIRECTORY_SEPARATOR . str_replace(
                '.',
                DIRECTORY_SEPARATOR,
                $controllerID
            );
    ...
    public function generateController($source, $params)
    {
        ...
        if ($id === null) {
            throw new ShellException(Yii::t(
                'yii',
                'Error: Table "{table}" does not have a primary key.',
                array('{table}' => $model->tableName())
            ));
        } elseif (is_array($id)) {
            throw new ShellException(Yii::t(
                'yii',
