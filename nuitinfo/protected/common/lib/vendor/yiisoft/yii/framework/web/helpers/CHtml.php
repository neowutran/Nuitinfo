...
class CHtml
{
        ...
        if (!strcasecmp($method, 'get') && ($pos = strpos($url, '?')) !== false) {
            foreach (explode('&', substr($url, $pos + 1)) as $pair) {
                if (($pos = strpos($pair, '=')) !== false) {
                    $hiddens[] = self::hiddenField(
                        urldecode(substr($pair, 0, $pos)),
                        urldecode(substr($pair, $pos + 1)),
                        array('id' => false)
                    );
                } else {
    ...
    public static function textArea($name, $value = '', $htmlOptions = array())
    {
        ...
        return self::tag(
            'textarea',
            $htmlOptions,
            isset($htmlOptions['encode']) && !$htmlOptions['encode'] ? $value : self::encode($value)
        );
    }
    ...
    public static function checkBoxList($name, $select, $data, $htmlOptions = array())
    {
        ...
        foreach ($data as $value => $labelTitle) {
            $checked              = !is_array($select) && !strcmp($value, $select) || is_array($select) && in_array(
                    $value,
                    $select
                );
    ...
    public static function listOptions($selection, $listData, &$htmlOptions)
    {
        ...
        if (isset($htmlOptions['empty'])) {
            ...
            foreach ($htmlOptions['empty'] as $value => $label) {
                $content .= '<option value="' . self::encode($value) . '">' . strtr(
                        $label,
                        array('<' => '&lt;', '>' => '&gt;')
                    ) . "</option>\n";
            }
        ...
        foreach ($listData as $key => $value) {
            if (is_array($value)) {
                ...
                if (!is_array($selection) && !strcmp($key, $selection) || is_array($selection) && in_array(
                        $key,
                        $selection
                    )
                ) {
                ...
                $content .= self::tag(
                        'option',
                        $attributes,
                        $raw ? (string)$value : self::encode((string)$value)
                    ) . "\n";
            }
        }
    ...
    public static function resolveName($model, &$attribute)
    {
        ...
        if (($pos = strpos($attribute, '[')) !== false) {
            ...
            if (preg_match('/\](\w+\[.*)$/', $attribute, $matches)) {
                $name      = $modelName . '[' . str_replace(
                        ']',
                        '][',
                        trim(strtr($attribute, array('][' => ']', '[' => ']')), ']')
                    ) . ']';
                $attribute = $matches[1];
                return $name;
            }
