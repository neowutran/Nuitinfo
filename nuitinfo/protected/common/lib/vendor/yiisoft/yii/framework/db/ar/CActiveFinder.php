...
class CJoinElement
{
        ...
        if ($this->relation instanceof CManyManyRelation) {
            ...
            foreach ($fks as $i => $fk) {
                if (isset($joinTable->foreignKeys[$fk])) // FK defined
                {
                    ...
                    if (!isset($parentCondition[$pk]) && $schema->compareTableNames(
                        ...
                        $count++;
                    } elseif (!isset($childCondition[$pk]) && $schema->compareTableNames(
                            $this->_table->rawName,
                            $tableName
                        )
                    ) {
            ...
            if (!$fkDefined) {
                ...
                foreach ($fks as $i => $fk) {
                    if ($i < count($parent->_table->primaryKey)) {
                        ...
                        ) ? $parent->_table->primaryKey[$i] : $parent->_table->primaryKey;
                        $parentCondition[$pk]    = $joinAlias . '.' . $schema->quoteColumnName($fk) . '=:ypl' . $count;
                        $params[':ypl' . $count] = $record->$pk;
                        $count++;
                    } else {
                        ...
                        ) ? $this->_table->primaryKey[$j] : $this->_table->primaryKey;
                        $childCondition[$pk] = $this->getColumnPrefix() . $schema->quoteColumnName(
                                $pk
                            ) . '=' . $joinAlias . '.' . $schema->quoteColumnName($fk);
                    }
            ...
            if ($parentCondition !== array() && $childCondition !== array()) {
                ...
                $join .= '(' . implode(') AND (', $parentCondition) . ') AND (' . implode(
                        ') AND (',
                        $childCondition
                    ) . ')';
            ...
            $fks    = is_array($element->relation->foreignKey) ? $element->relation->foreignKey : preg_split(
                '/\s*,\s*/',
                $element->relation->foreignKey,
                -1,
                PREG_SPLIT_NO_EMPTY
    ...
    public function getColumnSelect($select = '*')
    {
        ...
        if ($select === '*') {
            foreach ($this->_table->getColumnNames() as $name) {
                $columns[] = $prefix . $schema->quoteColumnName($name) . ' AS ' . $schema->quoteColumnName(
            ...
            if (is_string($this->_pkAlias) && !isset($selected[$this->_pkAlias])) {
                $columns[] = $prefix . $schema->quoteColumnName(
                        $this->_table->primaryKey
                    ) . ' AS ' . $schema->quoteColumnName($this->_pkAlias);
    ...
    public function getPrimaryKeyRange()
    {
        ...
        return $this->_builder->createInCondition(
            $this->_table,
            $this->_table->primaryKey,
            $values,
    ...
    public function getJoinCondition()
    {
        ...
        if ($this->relation instanceof CManyManyRelation) {
            ...
            $fks = is_array($this->relation->foreignKey) ? $this->relation->foreignKey : preg_split(
                '/\s*,\s*/',
    ...
    private function joinOneMany($fke, $fks, $pke, $parent)
    {
        ...
        foreach ($fks as $i => $fk) {
            ...
            if (is_int($i)) {
                if (isset($fke->_table->foreignKeys[$fk]) && $schema->compareTableNames(
                        $pke->_table->rawName,
...
class CStatElement
{
    ...
    private function queryOneMany()
    {
        ...
        if (count($fks) === 1) // single column FK
        {
            ...
            $sql     = "SELECT $col AS $c, {$relation->select} AS $s FROM {$table->rawName} " . $tableAlias . $join
                . $where . '(' . $builder->createInCondition(
                    $table,
                    $fks[0],
                    array_keys($records),
                    $tableAlias . '.'
        ...
        } else // composite FK
        {
            ...
            $sql     = 'SELECT ' . implode(
                    ', ',
                    $cols
                ) . ", {$relation->select} AS $s FROM {$table->rawName} " . $tableAlias . $join
        ...
        foreach ($records as $pk => $record) {
            $record->addRelatedRecord(
                $relation->name,
                isset($stats[$pk]) ? $stats[$pk] : $relation->defaultValue,
                false
            );
        }
    }
    ...
    private function queryManyMany($joinTableName, $keys)
    {
        ...
        foreach ($fks as $i => $fk) {
            ...
            if (isset($joinTable->foreignKeys[$fk])) {
                ...
                if (!isset($joinCondition[$pk]) && $schema->compareTableNames($table->rawName, $tableName)) {
                    $joinCondition[$pk] = $tableAlias . '.' . $schema->quoteColumnName(
                            $pk
                        ) . '=' . $joinTable->rawName . '.' . $schema->quoteColumnName($fk);
        ...
        foreach ($records as $pk => $record) {
            $record->addRelatedRecord(
                $relation->name,
                isset($stats[$pk]) ? $stats[$pk] : $this->relation->defaultValue,
                false
            );
        }
    }
