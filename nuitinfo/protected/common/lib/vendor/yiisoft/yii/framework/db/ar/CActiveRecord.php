...
abstract class CActiveRecord extends CModel
{
    ...
    public function getRelated($name, $refresh = false, $params = array())
    {
        ...
        if ($this->getIsNewRecord(
            ) && !$refresh && ($relation instanceof CHasOneRelation || $relation instanceof CHasManyRelation)
        ) {
    ...
    protected function query($criteria, $all = false)
    {
        ...
        if (empty($criteria->with)) {
            ...
            $command = $this->getCommandBuilder()->createFindCommand(
                $this->getTableSchema(),
                $criteria,
                $this->getTableAlias()
            );
            return $all ? $this->populateRecords($command->queryAll(), true, $criteria->index) : $this->populateRecord(
                $command->queryRow()
            );
        } else {
    ...
    public function findByAttributes($attributes, $condition = '', $params = array())
    {
        ...
        $prefix   = $this->getTableAlias(true) . '.';
        $criteria = $this->getCommandBuilder()->createColumnCriteria(
            $this->getTableSchema(),
            $attributes,
            $condition,
            $params,
    ...
    public function mergeWith($criteria, $fromScope = false)
    {
        ...
        if (isset($criteria['select']) && $this->select !== $criteria['select']) {
            if ($this->select === '*') {
                ...
                $select1      = is_string($this->select) ? preg_split(
                    '/\s*,\s*/',
                    trim($this->select),
                    -1,
                    PREG_SPLIT_NO_EMPTY
                ) : $this->select;
                $select2      = is_string($criteria['select']) ? preg_split(
                    '/\s*,\s*/',
                    trim($criteria['select']),
                    -1,
                    PREG_SPLIT_NO_EMPTY
                ) : $criteria['select'];
