...
class CHttpRequest extends CApplicationComponent
{
    ...
    public function getScriptUrl()
    {
        ...
        if ($this->_scriptUrl === null) {
            ...
            } elseif (isset($_SERVER['DOCUMENT_ROOT']) && strpos(
    ...
    public function getIsFlashRequest()
    {
        ...
        return isset($_SERVER['HTTP_USER_AGENT']) && (stripos(
                $_SERVER['HTTP_USER_AGENT'],
                'Shockwave'
            ) !== false || stripos($_SERVER['HTTP_USER_AGENT'], 'Flash') !== false);
    }
    ...
    public static function compareAcceptTypes($a, $b)
    {
        ...
        if ($a['params']['q'] === $b['params']['q']) {
            if (!($a['type'] === '*' xor $b['type'] === '*')) {
                if (!($a['subType'] === '*' xor $b['subType'] === '*')) // finally, higher number of parameters counts as greater precedence
                {
    ...
    public function getPreferredLanguages()
    {
        ...
        if ($this->_preferredLanguages === null) {
            ...
            if (isset($_SERVER['HTTP_ACCEPT_LANGUAGE']) && $n = preg_match_all(
                    '/([\w\-_]+)(?:\s*;\s*q\s*=\s*(\d*\.?\d*))?/',
                ...
                usort(
                    $languages,
                    create_function('$a,$b', 'if($a[0]==$b[0]) {return 0;} return ($a[0]<$b[0]) ? 1 : -1;')
                );
    ...
    public function sendFile($fileName, $content, $mimeType = null, $terminate = true)
    {
        ...
        $content = function_exists('mb_substr') ? mb_substr($content, $contentStart, $length) : substr(
            $content,
            $contentStart,
            $length
        );
