        ...
        if (empty($directions)) {
            ...
            foreach ($directions as $attribute => $descending) {
                ...
                if (is_array($definition)) {
                    if ($descending) {
                        $orders[] = isset($definition['desc']) ? (is_array($definition['desc']) ? implode(
                            ', ',
                            $definition['desc']
                        ) : $definition['desc']) : $attribute . ' DESC';
                    } else {
                        $orders[] = isset($definition['asc']) ? (is_array($definition['asc']) ? implode(
                            ', ',
                            $definition['asc']
                        ) : $definition['asc']) : $attribute;
                    }
                    ...
                    if (isset($schema)) {
                        if (($pos = strpos($attribute, '.')) !== false) {
                            $attribute = $schema->quoteTableName(
                                    substr($attribute, 0, $pos)
                                ) . '.' . $schema->quoteColumnName(substr($attribute, $pos + 1));
                        } else {
                            $attribute = ($criteria === null || $criteria->alias === null ? $this->getModel(
                                    $this->modelClass
                                )->getTableAlias(true) : $schema->quoteTableName(
                                    $criteria->alias
