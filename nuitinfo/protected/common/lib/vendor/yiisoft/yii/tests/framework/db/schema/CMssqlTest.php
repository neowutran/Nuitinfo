...
class CMssqlTest extends CTestCase
{
    ...
    public function setUp()
    {
        ...
        if (self::DB_DSN_PREFIX == 'sqlsrv' && (!extension_loaded('pdo') || !extension_loaded(
                    'sqlsrv'
        ...
        try {
            ...
            $this->markTestSkipped(
                "Please read $schemaFile for details on setting up the test environment for MSSQL test case."
            );
        }
...
EOD;
    ...
    public function testTable()
    {
        ...
        $this->assertEquals('id', $table->primaryKey);
        $this->assertEquals(
            array('col1' => array('orders', 'key1'), 'col2' => array('orders', 'key2')),
            $table->foreignKeys
        );
    ...
    protected function checkColumns($tableName, $values)
    {
        ...
        foreach ($values as $name => $value) {
            foreach (array_values($table->columns) as $i => $column) {
                ...
                $this->assertTrue(
                    $column->$name == $value[$i],
                    "$tableName.{$column->name}.$name is {$column->$name} ($type1), different from the expected {$value[$i]} ($type2)."
                );
            }
        }
    }
    ...
    public function testCommandBuilder()
    {
        ...
        $c = $builder->createInsertCommand(
            $table,
            array('title' => 'test post', 'create_time' => '2000-01-01', 'author_id' => 1, 'content' => 'test content')
        );
        $this->assertEquals(
            'INSERT INTO [dbo].[posts] ([title], [create_time], [author_id], [content]) VALUES (:yp0, :yp1, :yp2, :yp3)',
            $c->text
        );
        $c->execute();
        ...
        $c = $builder->createUpdateCommand(
            $table,
            array('title' => 'new post 5'),
            new CDbCriteria(array(
                'condition' => 'id=:id',
                'params'    => array('id' => 5)
            ))
        ...
        $c = $builder->createUpdateCounterCommand(
            $table,
            array('author_id' => -2),
            new CDbCriteria(array('condition' => 'id=5'))
        );
        ...
        $c = $builder->createPkCriteria(
            $table2,
            array(array('key1' => 1, 'key2' => 2), array('key1' => 3, 'key2' => 4))
        );
        $this->assertEquals(
            '(([dbo].[orders].[key1]=1 AND [dbo].[orders].[key2]=2) OR ([dbo].[orders].[key1]=3 AND [dbo].[orders].[key2]=4))',
            $c->condition
        );
        ...
        $this->assertEquals(
            '[dbo].[posts].[id]=:yp0 AND [dbo].[posts].[author_id]=:yp1 AND (title=\'\')',
            $c->condition
        );
    ...
    public function testTransactions()
    {
        ...
        try {
            $builder->createInsertCommand(
                $table,
                array(
                    'title'       => 'working transaction test post 1',
                    'create_time' => '2009-01-01',
                    'author_id'   => 1,
            ...
            $builder->createInsertCommand(
                $table,
                array(
                    'title'       => 'working transaction test post 2',
                    'create_time' => '2009-01-01',
                    'author_id'   => 1,
                    'content'     => 'test content'
                )
        ...
        $n = $builder->createCountCommand(
            $table,
            new CDbCriteria(array('condition' => "title LIKE 'working transaction%'"))
        )->queryScalar();
        ...
        try {
            $builder->createInsertCommand(
                $table,
                array(
                    'title'       => 'failed transaction test post 1',
                    'create_time' => '2009-01-01',
                    'author_id'   => 1,
                    'content'     => 'test content'
                )
            ...
            $builder->createInsertCommand(
                ...
                array(
                    'id'          => 1,
                    'title'       => 'failed transaction test post 2',
                    'create_time' => '2009-01-01',
                    'author_id'   => 1,
                    'content'     => 'test content'
                )
        ...
        $n = $builder->createCountCommand(
            $table,
            new CDbCriteria(array('condition' => "title LIKE 'failed transaction%'"))
        )->queryScalar();
    ...
    public function testResetSequence()
    {
        ...
        $this->db->createCommand()->insert(
            'users',
            array('username' => 'testerX', 'password' => 'passwordX', 'email' => 'emailX@gmail.com')
        );
        ...
        $this->db->createCommand()->insert(
            'users',
            array('username' => 'testerY', 'password' => 'passwordY', 'email' => 'emailY@gmail.com')
        );
        ...
        $this->db->createCommand()->insert(
            'users',
            array('username' => 'testerZ', 'password' => 'passwordZ', 'email' => 'emailZ@gmail.com')
        );
