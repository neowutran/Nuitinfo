...
class COciTest extends CTestCase
{
    ...
    public function setUp()
    {
        ...
        if ((!extension_loaded('oci8') && !extension_loaded('oci8_11g')) || !extension_loaded(
                'pdo'
        ...
        try {
            ...
            $this->markTestSkipped(
                "Please read {$schemaFilePath} for details on setting up the test environment for OCI test case."
            );
        }
...
EOD;
    ...
    public function testCommandBuilder()
    {
        ...
        $c = $builder->createInsertCommand(
            ...
            array(
                'title'       => 'test post',
                'create_time' => new CDbExpression('TO_TIMESTAMP(:ts_value, \'YYYY-MM-DD\')', array(':ts_value' => '2000-01-01')),
                'author_id'   => 1,
                'content'     => 'test content'
            )
        );
        $this->assertEquals(
            'INSERT INTO "posts" ("title", "create_time", "author_id", "content") VALUES (:yp0, TO_TIMESTAMP(:ts_value, \'YYYY-MM-DD\'), :yp1, :yp2) RETURNING "id" INTO :RETURN_ID',
            $c->text
        );
        $c->execute();
        ...
        $this->assertEquals(
            'WITH USER_SQL AS (SELECT "id", "title" FROM "posts" "t" WHERE "id"=:id ORDER BY "title"),
    ...
    public function testMultipleInsert()
    {
        ...
        $rows = $builder->dbConnection->createCommand(
