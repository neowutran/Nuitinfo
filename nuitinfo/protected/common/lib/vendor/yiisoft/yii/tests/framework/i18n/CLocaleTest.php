...
class CLocaleTest extends CTestCase
{
    ...
    function testRequiredDataExistence()
    {
        ...
        foreach ($this->criticalLocaleCodes as $localeCode) {
            ...
            $this->assertNotNull(
                $locale->getMonthName(12, 'abbreviated'),
                "$localeCode: getMonthName 12 abbreviated failed."
            );
            $this->assertNotNull(
                $locale->getMonthName(1, 'narrow', true),
                "$localeCode: getMonthName 1 narrow standalone failed."
            );
            ...
            // ORIENTATION
            $this->assertTrue(
                in_array($locale->getOrientation(), array('ltr', 'rtl')),
                "$localeCode: getOrientation failed."
            );
