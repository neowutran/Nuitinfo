...
class CDateTimeParserTest extends CTestCase
{
    ...
    public function testParseDefaults()
    {
        ...
        $this->assertEquals(
            ...
            date(
                'd-m-Y H:i:s',
                CDateTimeParser::parse('2011-12-31', 'yyyy-MM-dd', array('hour' => 23, 'minute' => 59, 'second' => 59))
            )
        );
    ...
    public function testFailing()
    {
        ...
        $this->assertFalse(
            CDateTimeParser::parse('yyyy-MM-dd', '2011-12-31', array('hour' => 23, 'minute' => 59, 'second' => 59))
        );
        ...
        // inexistent patterns
        $this->assertFalse(
            CDateTimeParser::parse('05, 1991, 01:09, mar', 'dd, yyyy, HH:mm, MMMMM')
        ); // illegal month pattern
        $this->assertFalse(
            CDateTimeParser::parse('05, 1991, 01:09, mar', 'ddd, yyyy, HH:mm, MMM')
        ); // illegal day pattern
        $this->assertFalse(
            CDateTimeParser::parse('05, 1991, 01:09, mar', 'dd, yyyyy, HH:mm, MMM')
        ); // illegal year pattern
    }
}
