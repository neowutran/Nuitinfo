...
class CMysqlTest extends CTestCase
{
    ...
    public function setUp()
    {
        ...
        try {
            ...
            $this->markTestSkipped(
                "Please read $schemaFile for details on setting up the test environment for MySQL test case."
            );
        }
    ...
    public function testCommandBuilder()
    {
        ...
        $this->assertEquals(
            'INSERT INTO `posts` (`title`, `create_time`, `author_id`, `content`) VALUES (:yp0, :yp1, :yp2, :yp3)',
            $c->text
        );
        $c->execute();
        ...
        $this->assertEquals(
            'DELETE `posts` FROM `posts` JOIN `users` u ON `author_id`=u.`id` WHERE u.`username`=:username and `posts`.`title`=:title',
            $c->text
        );
        $c->execute();
        $c = $builder->createCountCommand($table, new CDbCriteria);
        ...
        $this->assertEquals(
            'UPDATE `posts` JOIN `users` u ON `author_id`=u.`id` SET `author_id`=`author_id`-1 WHERE u.`username`="user2"',
            $c->text
        );
        $c->execute();
