...
class CDbCriteriaTest extends CTestCase
{
    ...
    public function testMergeWith()
    {
        ...
        $this->assertEquals(
            array(array('scope' => array(1, 2)), 'scope1', 'scope2', array('scope' => array(3, 4))),
            $criteria1->scopes
        );
        ...
        $this->assertEquals(
            array(
                array('scope' => array(1, 2)),
                array('scope' => array(3, 4)),
                array('scope' => array(5, 6)),
                array('scope' => array(7, 8))
            ),
            $criteria1->scopes
        );
        ...
        $this->assertEquals(
            '((((((A1=:ycp0) AND (A2=:ycp1)) AND (A3=:ycp2)) AND (A4=:ycp3)) AND (A5=:ycp4)) AND (A6=:ycp5)) AND ((((((B1=:ycp6) AND (B2=:ycp7)) AND (B3=:ycp8)) AND (B4=:ycp9)) AND (B5=:ycp10)) AND (B6=:ycp11))',
            $criteria1->condition
        );
    ...
    public function testSerializeAutomaticallyGeneratedParams()
    {
        ...
        $criteria                     = new CDbCriteria();
        $paramName                    = CDbCriteria::PARAM_PREFIX . rand(
                10000,
                20000
            ); // mock up automatically generated name
        $paramValue                   = 'testParamValue';
        ...
        $this->assertArrayNotHasKey(
            $paramName,
            $unserializedCriteria->params,
            'Param name which match automatic generation has not been replaced!'
        );
        $this->assertContains(
            $paramValue,
            $unserializedCriteria->params,
            'Automatically generated param value has been lost!'
        );

        $newParamName = array_search($paramValue, $unserializedCriteria->params, true);
        $this->assertEquals(
            str_replace($paramName, $newParamName, $criteria->condition),
            $unserializedCriteria->condition,
            'Criteria condition has not been updated!'
        );
        $this->assertEquals(
            str_replace($paramName, $newParamName, $criteria->order),
            $unserializedCriteria->order,
            'Criteria order has not been updated!'
        );
        $this->assertEquals(
            str_replace($paramName, $newParamName, $criteria->group),
            $unserializedCriteria->group,
            'Criteria group has not been updated!'
        );
        $this->assertEquals(
            str_replace($paramName, $newParamName, $criteria->having),
            $unserializedCriteria->having,
            'Criteria having has not been updated!'
        );
        $this->assertEquals(
            str_replace($paramName, $newParamName, $criteria->select),
            $unserializedCriteria->select,
            'Criteria select has not been updated!'
        );
    }
