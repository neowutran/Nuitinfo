...
class CSqliteTest extends CTestCase
{
    ...
    public function testCommandBuilder()
    {
        ...
        $c = $builder->createInsertCommand(
            $table,
            array('title' => 'test post', 'create_time' => time(), 'author_id' => 1, 'content' => 'test content')
        );
        $this->assertEquals(
    ...
    public function testCheckIntegrity2()
    {
        ...
        $this->assertEquals(
            0,
            $this->db->createCommand('SELECT COUNT(*) FROM profiles WHERE user_id=9999')->queryScalar()
        );
        if (version_compare(PHP_VERSION, '5.3.0', '>=')) {
            $this->setExpectedException('CDbException');
        }
        $this->db->createCommand(
            "INSERT INTO profiles (first_name,last_name,user_id) VALUES ('orphaned','profile',9999)"
        )->execute();
        $this->assertEquals(
            1,
            $this->db->createCommand('SELECT COUNT(*) FROM profiles WHERE user_id=9999')->queryScalar()
