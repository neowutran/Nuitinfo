...
class CActiveRecordTest extends CTestCase
{
    ...
    public function testModel()
    {
        ...
        $this->assertEquals(
            array('id' => null, 'title' => null, 'create_time' => null, 'author_id' => null, 'content' => null),
            $model->attributes
        );
    ...
    public function testRelationWithCondition()
    {
        ...
        $this->assertTrue($post instanceof Post);
        $posts = Post::model()->with('comments')->findAllBySql(
            'select * from posts where id=:id1 OR id=:id2',
            array(':id1' => 2, ':id2' => 3)
        );
        $this->assertEquals(2, count($posts));
    ...
    public function testScopeWithRelations()
    {
        ...
        $user6 = User::model()->findByPk(
            2,
            array('with' => array('posts' => array('scopes' => 'post23A', 'alias' => 'alias')))
        );
        ...
        $user6 = User::model()->findByPk(
            2,
            array('with' => array('posts' => array('scopes' => array('p' => array(4)))))
        );
        $user7 = User::model()->findByPk(
            2,
            array('with' => array('posts' => array('scopes' => array(array('p' => 4)))))
        );
        $user8 = User::model()->findByPk(
            2,
            array('with' => array('posts' => array('scopes' => array(array('p' => array(4))))))
        );
        ...
        $user4 = User::model()->with(
            array('posts' => array('scopes' => array(array('behaviorP' => array(4)))))
        )->findByPk(2);
        $user5 = User::model()->findByPk(
            2,
            array('with' => array('posts' => array('scopes' => array('behaviorP' => 4))))
        );
        $user6 = User::model()->findByPk(
            2,
            array('with' => array('posts' => array('scopes' => array('behaviorP' => array(4)))))
        );
        $user7 = User::model()->findByPk(
            2,
            array('with' => array('posts' => array('scopes' => array(array('behaviorP' => 4)))))
        );
        $user8 = User::model()->findByPk(
            2,
            array('with' => array('posts' => array('scopes' => array(array('behaviorP' => array(4))))))
        );
    ...
    public function testLazyLoadThroughRelationWithCondition()
    {
        ...
        $this->assertEquals(
            0,
            count($masterModel->usersWhichEmptyByCondition),
            'Unable to apply condition from through relation!'
        );
    }
