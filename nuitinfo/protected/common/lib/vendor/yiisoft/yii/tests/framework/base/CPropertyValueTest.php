...
class CPropertyValueTest extends CTestCase
{
    ...
    public function testEnsureBoolean()
    {
        ...
        foreach ($entries as $index => $entry) {
            $this->assertTrue(
                CPropertyValue::ensureBoolean($entry[0]) === $entry[1],
    ...
    public function testEnsureString()
    {
        ...
        foreach ($entries as $index => $entry) {
            $this->assertTrue(
                CPropertyValue::ensureString($entry[0]) === $entry[1],
                "Comparison $index: {$this->varToString($entry[0])}=={$this->varToString($entry[1])}"
            );
        }
    }
    ...
    public function testEnsureInteger()
    {
        ...
        foreach ($entries as $index => $entry) {
            $this->assertTrue(
                CPropertyValue::ensureInteger($entry[0]) === $entry[1],
                "Comparison $index: {$this->varToString($entry[0])}=={$this->varToString($entry[1])}"
            );
        }
    }
    ...
    public function testEnsureFloat()
    {
        ...
        foreach ($entries as $index => $entry) {
            $this->assertTrue(
                CPropertyValue::ensureFloat($entry[0]) === $entry[1],
                "Comparison $index: {$this->varToString($entry[0])}=={$this->varToString($entry[1])}"
            );
        }
    }
    ...
    public function testEnsureArray()
    {
        ...
        foreach ($entries as $index => $entry) {
            $this->assertTrue(
                CPropertyValue::ensureArray($entry[0]) === $entry[1],
                "Comparison $index: {$this->varToString($entry[0])}=={$this->varToString($entry[1])}"
            );
        }
    }
