...
class CActiveRecord2Test extends CTestCase
{
    ...
    public function testLazyRelation()
    {
        ...
        $this->assertEquals(
            ...
            $post->author->getAttributes(false)
        );
        ...
        $this->assertEquals(
            ...
            $post->firstComment->getAttributes(false)
        );
        ...
        $this->assertEquals(
            ...
            $post->comments[0]->getAttributes(false)
        );
        $this->assertEquals(
            array(
                'id'        => 4,
            ...
            $post->comments[1]->getAttributes(false)
        );
        $post = Post2::model()->findByPk(4);
        ...
        $this->assertEquals(
            ...
            $post->categories[0]->getAttributes(false)
        );
        $this->assertEquals(
            array(
                'id'        => 1,
            ...
            $post->categories[1]->getAttributes(false)
        );
        ...
        $this->assertEquals(
            ...
            $category->children[0]->getAttributes(false)
        );
        $this->assertEquals(
            array(
                'id'        => 7,
            ...
            $category->children[1]->getAttributes(false)
        );
        ...
        $this->assertEquals(
            ...
            $category->parent->getAttributes(false)
        );
        ...
        $this->assertEquals(
            ...
            $item->order->getAttributes(false)
        );
    }
    ...
    public function testLazyRecursiveRelation()
    {
        ...
        $this->assertEquals(
            2,
            count($category->nodes[0]->children)
        ); // row in test.categories with id 5 (has 2 descendants)
        $this->assertEquals(
            0,
            count($category->nodes[1]->children)
    ...
    public function testRelationWithCondition()
    {
        ...
        $this->assertTrue($post instanceof Post2);
        $posts = Post2::model()->with('comments')->findAllBySql(
            'select * from test.posts where id=:id1 OR id=:id2',
            array(':id1' => 2, ':id2' => 3)
        );
        $this->assertEquals(2, count($posts));
    ...
    public function testEmptyFinding()
    {
        ...
        $post = Post2::model()->with('author', 'firstComment', 'comments', 'categories')->findBySql(
            'SELECT * FROM test.posts WHERE id=100'
        );
        $this->assertNull($post);

        Post2::model()->with('author', 'firstComment', 'comments', 'categories')->findAllBySql(
            'SELECT * FROM test.posts WHERE id=100'
        );
        $this->assertTrue($posts === array());
    }
    ...
    public function testEmptyModel()
    {
        ...
        $this->assertEquals(
            ...
            $this->db->createCommand(
                'SELECT COUNT(*) FROM "test"."nullable_posts" WHERE LENGTH("title") > 0'
            )->queryScalar()
        );
    }
}
