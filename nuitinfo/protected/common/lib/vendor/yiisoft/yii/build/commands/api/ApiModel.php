...
class ApiModel
{
    ...
    protected function processMethod($class, $method)
    {
        ...
        foreach ($doc->input as $param) {
            ...
            if ($param->isOptional) {
                $params[] = $type . ($param->isPassedByReference ? '&' : '') . '$' . $param->name . '=' . str_replace(
                        "\r",
                        '',
        ...
        $doc->signature = '{{' . $class->name . '::' . $doc->name . '|<b>' . $doc->name . '</b>}}(' . implode(
                ', ',
                $params
            ) . ')';
        if ($doc->output !== null) {
    ...
    protected function isValidPath($path)
    {
        ...
        foreach ($this->_excludes as $exclude) {
            if (($exclude[0] === '/' && $this->_sourcePath . $exclude === $path) || ($exclude[0] !== '/' && basename(
                        $path
                    ) === $exclude)
            ) {
                return false;
