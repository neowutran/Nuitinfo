...
EOD;
    ...
    public function run($args)
    {
        ...
        if (!is_file(
            $pluralFile = $basePath . DIRECTORY_SEPARATOR . 'supplemental' . DIRECTORY_SEPARATOR . 'plurals.xml'
        )
        ) {
...
EOD;
        ...
        file_put_contents(
            $i18nDataPath . DIRECTORY_SEPARATOR . strtolower($locale) . '.php',
            "<?php\n" . $content . "\n"
        );
    ...
    protected function parseTerritories($xml, &$data)
    {
        ...
        foreach ($territories as $territory) {
            if ((string)$territory != '') {
                $data['territories'][strtolower(
                    str_replace('-', '_', (string)$territory['type'])
                )] = (string)$territory;
            }
        }
    }
    ...
    protected function parseDateTimeFormat($xml, &$data)
    {
        ...
        $types = $xml->xpath(
            '/ldml/dates/calendars/calendar[@type=\'gregorian\']/dateTimeFormats/dateTimeFormatLength'
        );
    ...
    protected function parsePluralRules($xml)
    {
        ...
        $patterns = array(
            ...
            '/\s+is\s+/i'                                      => '==',
            //is
            '/n\s+mod\s+(\d+)/i'                               => 'fmod(n,$1)',
            //mod (CLDR's "mod" is "fmod()", not "%")
            '/^(.*?)\s+not\s+(?:in|within)\s+(\d+)\.\.(\d+)/i' => '($1<$2||$1>$3)',
            //not in, not within
            '/^(.*?)\s+within\s+(\d+)\.\.(\d+)/i'              => '($1>=$2&&$1<=$3)',
            //within
            '/^(.*?)\s+in\s+(\d+)\.\.(\d+)/i'                  => '($1>=$2&&$1<=$3&&fmod($1,1)==0)',
            //in
        );
